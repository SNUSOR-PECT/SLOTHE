...
if [[ $cnt != 0 ]]; then
  ...
  bash ./scripts/cf_optimizer_unreachable.sh $1 $3 $4

  # 3. CF-Optimizer (2) Equivalent path merging
  # TODO: run cf_optimizer_pathMerge.sh until all branches are analyzed
  bash ./scripts/cf_optimizer_pathMerge.sh $1 _$1_Unreachable _$1_Merged $2

  # 4. Check the validity of optimized IRB compared to initial IRB
  bash ./scripts/checkFunctionalValid.sh $1 _$1_Merged $2 $3 $4

  # 5. (Optional) Apply additional optimization
  if [[ $1 == "tanh" ]]; then
      bash ./scripts/optional.sh $1 _$1_Merged $1_optim
      bash ./scripts/checkFunctionalValid.sh $1 $1_optim $2 $3 $4
  else
